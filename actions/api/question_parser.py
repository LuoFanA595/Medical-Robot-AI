class QuestionPaser:
    ...
    def sql_transfer(self, question_type, entities):
        ...
        # 查询疾病的并发症
            ...
            sql1 = ["MATCH (m:Disease)-[r:acompany_with]->(n:Symptom) where m.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
            sql2 = ["MATCH (m:Disease)-[r:acompany_with]->(n:Symptom) where n.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
            sql = sql1 + sql2
        # 查询疾病的忌口
        elif question_type == 'disease_not_food':
